...
const handler = async (m, {conn, args, usedPrefix, command}) => {
  ...
  if (args[0].includes('you')) {
    ...
    if (index >= 0) {
      if (Array.isArray(global.videoList) && global.videoList.length > 0) {
        ...
        enviando = false  
        throw `*[❗] To be able to make use of the command in this way (${usedPrefix + command} <numero>), Please perform the video search with the ${usedPrefix}playlist <text>*`;
      }
    }
  }
  ...
  try {
    ...
    try {
      ...
      await conn.sendMessage(m.chat, {video: {url: mediaa.result}, fileName: `error.mp4`, caption: `_Mimi - 𝐁𝐨𝐭_`, thumbnail: mediaa.thumb, mimetype: 'video/mp4'}, {quoted: m});
      await conn.sendMessage(m.chat, {text: '*[ ✅ ] Video successfully downloaded.*', edit: key}, {quoted: m});
      enviando = false
    } catch {
      try {
